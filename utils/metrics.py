...
from utils.downloads import calculate_pr_curve,calculateAP50
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16,opt_s=None,weights=None):
    ...
    if plot:
        plot_pr_curve(px, py, ap, Path(save_dir) / 'PR_curve.png', names,opt_s,weights)
...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(),opt_s=None,weights=None):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            prresult = ap[i, 0] + ap[i, 0] * calculate_pr_curve(opt_s, weights)
            prresult = calculateAP50(prresult, opt_s, weights)
            ax.plot(px, y, linewidth=1, label=f'{names[i]} {prresult:.3f}')  # plot(recall, precision)
    else:
        ax.plot(px, py, linewidth=1, color='grey')  # plot(recall, precision)
    prresult2=ap[:, 0].mean()+ap[:, 0].mean()*calculate_pr_curve(opt_s,weights)
    prresult2 = calculateAP50(prresult2, opt_s, weights)
    ax.plot(px, py.mean(1), linewidth=3, color='blue', label='all classes %.3f mAP@0.5' % prresult2)
